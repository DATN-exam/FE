...
function Header() {
  ...
  return (
    <ul className="flex flex-wrap border-b border-gray-200 text-center text-sm font-medium text-gray-500 dark:border-gray-700 dark:text-gray-400">
      <li className="me-2">
        <Link
          to={ROUTES_TEACHER.SET_QUESTION.UPDATE.replace(':id', id ?? '')}
          className={cx(
            currentPath == ROUTES_TEACHER.SET_QUESTION.UPDATE.replace(':id', id ?? '')
              ? classNameActive
              : classNameInActive,
          )}
        >
      ...
      <li className="me-2">
        <Link
          to={ROUTES_TEACHER.SET_QUESTION.QUESTIONS.replace(':id', id ?? '')}
          className={cx(
            currentPath == ROUTES_TEACHER.SET_QUESTION.QUESTIONS.replace(':id', id ?? '')
              ? classNameActive
              : classNameInActive,
          )}
        >
